...
页面用五个标签页组织卡组（顺序固定）：**`1v1`** → **`HOT 1v1`** → **`决斗卡组`** → **`2v2`** → **`HOT 2v2`**。

- `1v1`（我的卡组）是默认页：全新用户显示空态引导（不再放示例卡组），卡片有 复制 / 编辑 / 删除；
- `HOT 1v1`（热门1v1卡组，内置 120 套）：卡片是 **复制 / 编辑**（**没有删除键**）；
- 站内复制：HOT 卡组点「复制」→ 弹出「复制到哪里？」选择框：
  - **1v1 · 我的卡组**（已实现，复制后自动切到 1v1 并提示，卡组含形态与塔防，重名自动加后缀）
  - **决斗卡组**（按钮存在，暂标「开发中」）；2v2 的 HOT 卡组将直接复制到 2v2
- 自己 1v1 里的卡组点「复制」= 在原地再复制一份（不弹选择框）；
- `决斗卡组` / `2v2` / `HOT 2v2` 目前显示「该功能正在开发中 · Coming Soon」占位。
